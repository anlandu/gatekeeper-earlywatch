...
- `tests/manualtouch/*` are legacy/illustrative fixtures from the old
  ...
  `library/gatekeeper-earlywatch/manual-touch-check/provider.yaml` + `touch-monitor/`.
...
- `ConstraintTemplate` count: `10` core parity templates (the ExpressionCheck
  example folder ships two templates: a plain equality predicate and a regex
  predicate).
...
2. **Runtime arbitrary-expression strings are intentionally not supported.**
   Gatekeeper compiles CEL and Rego at template creation and cannot `eval()`
   strings supplied as constraint parameters. Each EarlyWatch ExpressionCheck
   rule is translated into a dedicated `ConstraintTemplate` with a one-line
   CEL predicate; see `library/gatekeeper-earlywatch/expression-check/README.md`.
3. **Referential checks need inventory sync.** ExistingResources,
   ...
   require the relevant GVKs in `library/gatekeeper-earlywatch/config-sync.yaml`.
