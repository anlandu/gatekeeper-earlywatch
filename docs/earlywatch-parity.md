...
| **ExpressionCheck.** EarlyWatch evaluates expressions against admission context to deny matching changes. | `library/gatekeeper-earlywatch/expression-check/template.yaml` (`EWExpressionCheck`) provides a reusable structured expression surface: `operationIn`, `namespaceIn`, `namespaceRegex`, `nameIn`, and `nameRegex`. Rego denies when any configured predicate set matches. | The same structured template includes `K8sNativeValidation` for equivalent CEL enforcement of those structured predicates. | Intentional non-parity for runtime arbitrary-CEL expression strings: Gatekeeper compiles CEL at template creation and does not dynamically `eval()` CEL supplied as constraint parameters. Custom arbitrary CEL should be authored as dedicated `ConstraintTemplate`s outside the default parity stack. | `tests/expression/*`; `library/gatekeeper-earlywatch/expression-check/constraint-regex.yaml`; `gator verify tests/suite.yaml`. |
