#!/usr/bin/env bash
# EarlyWatch Azure Policy demo for rule: name-reference-check
# Renders an end-to-end deny-mode demo against the current az/kubectl context.
set -euo pipefail
RULE=name-reference-check
NS=ew-name-reference-check
DEFNAME=ew-namereferencecheck
...
if [ -f "$HERE/bad.yaml" ]; then
  log "greenfield BAD seed (CREATE cm+deployment allowed)"
  kubectl -n "$NS" apply -f "$HERE/bad.yaml"
  log "wait for web deployment to land in Gatekeeper sync inventory"
  sleep 60
  log "greenfield BAD DELETE referenced (expect deny: web deployment still references it and is in sync data)"
  if kubectl -n "$NS" delete cm referenced --wait=false 2>&1 | tee /tmp/$RULE.bad.log; then
